// src/pages/reports/ReportsCommentsPage.jsx
...
const ReportsCommentsPage = () => {
  ...
  const columns = [
    { key: "date", label: "Дата" },
    { key: "commentsCount", label: "Количество комментариев" },
  ];

  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(filteredComments);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Отчёт по комментариям");
    const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, `comments-report-${Date.now()}.xlsx`);
  };
